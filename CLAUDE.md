...
Project-specific context for agents working on neograph. Reading this saves you from re-discovering architecture, conventions, and hard-won lessons. For beads workflow and session-close protocol, see `AGENTS.md`.
...
**Website**: [neograph.pro](https://neograph.pro) (Astro + Starlight at `website/`, deployed via AWS Amplify on push to `main`). Don't forget to update website content when API surfaces change.
...
- **`main`** — stable. Only tagged releases and critical hotfix PRs. Currently at v0.1.0 (`a593a98`).
