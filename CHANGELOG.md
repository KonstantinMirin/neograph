...
- **BREAKING: a first-of-chain read with no producer is refused at assembly** (`neograph-rfp5s`). A top-level first node declaring single-type `inputs=X` -- or an `Each(over=...)` whose root names no producer -- used to assemble, compile and run green while the body received `None`: the compiled state has no field that could hold the value, `run(input=)` keys are dropped by the state model, and the validator tolerated the shape citing a runtime isinstance scan that `neograph-t1nbp` had already deleted. `Construct(...)` now raises `ConstructError` ("no upstream produces a compatible value") for that read, which is what the `@node` builder always did for the same shape ("parameter does not match any @node") -- three-surface parity, not a new rule. Every dead citation of the deleted scan is gone with it, including the `DeprecationWarning` rationale on single-type `inputs=` (it now says what the form does: resolve to the last compatible producer by type). Two shapes stay tolerated under named tickets: a child `input=` port at position 0 of a portless parent (`neograph-xejyn`, the top-level port has no public feeding spelling) and a Portal mesh member's single-type read (`neograph-sdqsv`).

  Two structural sites were what made the refusal look unsound, and both are fixed rather than exempted: the YAML/spec loader minted an implicit `inputs=<own output type>` read for every node without `inputs:`, which is unfeedable for the FIRST pipeline node and is now cleared there (later nodes keep the implicit same-type read); and the Agent Spec importer built a sub-flow's `Construct` portless and attached `input`/`output` afterwards with `model_copy`, which skips validation, so the child is now BUILT with its ports (`from_agent_spec(flow, _boundary=...)`).

  **Found in passing, filed**: `Node.input_from` naming a nonexistent member assembles and runs green with the body receiving `None` while a compatible producer sits unused (`neograph-3mqw4`, P1); a declarative `inputs=list[T]` after a Loop producer is validator-accepted but resolver-unstamped and receives `None` (`neograph-la3a4`, P1); the generic-alias and bare-`dict` forms of the same tolerance (`neograph-mkeul`); the runtime single-type read still returns `None` on absence rather than failing loud on a stamped `Source` (`neograph-z7fhl`).

- **Accumulator channels: `Accumulate[T]`** (`neograph-iq4a3`, closes [GH #16](https://github.com/KonstantinMirin/neograph/issues/16)). Many nodes -- including every branch of an `Each` -- append to one channel, and a downstream node reads the union. Declared where it is written, as a dict-form output key: `outputs={"result": Verdict, "readings": Accumulate[Reading]}`; read with `inputs={"readings": list[Reading]}` or a `@node` param of that name. The field is `readings`, unprefixed and shared, merged by the existing `_concat_reducer`. Before this a value could enter state only as a node's output, so accumulating across a fan cost a wrapper type, a scripted node whose only job was to push the branch's readings out, and a hand-written merge at the parent -- and the failure it caused was silent: a branch returning a bare `Claim` dropped every reading it took, by construction, and the terminal judge re-decided over the opening reads while each claim cited evidence that was gone. Green run. The reporter's wrapper type and parent merge are gone; a one-line scripted node remains inside an AGENT branch until `appends={channel: fn}` lands (filed).

  **Not a new mechanism, and deliberately not the Portal-scale shape.** The pre-registered `Accumulated` `Source` variant goes live, but there is no construct-level declaration, no new IR field and no new normalizer: `Accumulate[Reading]` is a plain generic alias that `_validate_type_spec` already accepts, stripped at `normalize_outputs` -- the one discriminator of `Node.outputs` -- into `list[Reading]` plus `NormalizedOutputs.accumulator_keys`. `contributed_fields` emits the channel as an unprefixed `Producer(is_accumulator=True)`, which is how the validator, the state builder, the fingerprint and lint all learn about it, once. Measured before choosing: 41 raw `.outputs` reads bypass the discriminator; 14 are Agent Spec objects or docstrings, 9 are log lines, 7 go through `primary_output_field`, and the rest are handled explicitly.

  **Ordering is defined**, not deterministic: ordered within a branch, unordered across branches, the same rule as the `list[X]`-consumer-of-Each caveat. Sort on a stable key you put in the element. **Refused at assembly**: two channels of one name with different element types; a channel colliding with a node field name; a channel as the primary key of a think/agent/act node; a channel on an Oracle- or Portal-modified node. **No representation, said aloud**: `to_agent_spec` fails loud for a construct with a channel, and `dump_spec` records it as an `accumulator_channel` loss rather than flattening it to a per-node `list[T]`.

  **Found in passing, filed**: a duplicate non-channel producer field still silently overwrites the earlier registration in `ArmScopedProducers.register` (the one sanctioned duplicate is now two type-agreeing channels; the rest is a latent seam of the `neograph-yz69e` class); and a FIRST node reading a channel nobody appends to is not refused, because a first node's dict-form inputs are not checked at all -- `neograph-rfp5s`, corroborated from a second direction.

- **One derivation of "which state fields does this item write"** (`neograph-yz69e`, absorbing `neograph-p93qh`). A `Portal(route="decide")` member writes its dispatched flow's typed result to a separate `{node}_dispatch` field. The VALIDATOR registered that field as a producer; **five other derivations of the same question did not**, and each disagreement was silent at a different seam. A downstream `inputs=Summary` (the documented single-type shorthand) type-checked green, `resolve_single_type_source` found no candidate, and the node was handed `None` at run time while a real typed `Summary` sat unread on the bus. Two further manifestations nobody had reported: a sub-construct whose `output=` is satisfied only by an inner dispatch member assembled green and died at run time with "No internal node produced a compatible output value", and a sub-construct `input=` port fed only by a dispatched result was never fed at all.

  **The fix is the missing monopoly, not the missing field.** neograph had already monopolised how to SPELL a state-field name (`output_field_name`) and how to READ one back (`split_output_field`), and never monopolised how to ENUMERATE them. `_ir_fields.contributed_fields(item) -> list[Producer]` is now that third monopoly: `declared_output_fields`, `item_field_names`, `_producer_pairs`, the validator's ~50-line registration block, `_schema_fingerprint._fingerprint_item` and lint's unconsumed-output walk are all projections of it. The validator's producer set and the resolver's candidate set are now the SAME OBJECT rather than two things asserted to agree — and the parity comments that asserted it, which were false when written, are deleted rather than reworded. Adding the field to the two functions the report named would have been a fifth copy.

  Three properties constrain the shape and each was load-bearing. It is **per-ITEM and never walks a construct**: three callers need three different arm policies (top-level only, arm-SCOPED, arm-inclusive flat), and a walking primitive would have silently destroyed the arm scoping that stops a false-arm node reading a true-arm field. It returns an **ordered list**, because `item_field_names` reads it last-declared-first for boundary precedence and order cannot be recovered from a set. And the record carries **both the declared and the effective type**, because validation type-checks the effective one (an `Each` producer writes `dict[str, X]`) while the fingerprint hashes the declared one — reading `effective_type` in the fingerprint projection would have changed every `Each` node's fingerprint and invalidated every existing checkpoint.

  `Producer` and `effective_producer_type`/`effective_producer_type_for` moved down from `_validation_types` to `_ir_fields` beside the enumeration that builds them: a producer record is a fact about what an item contributes, not a fact about validation. `_validation_types` imports and re-exports all three under an explicit `__all__`, so the validation cluster's public seam is unchanged. `StateKeys.dispatch()` joins its long-standing sibling `StateKeys.dispatch_error()`, replacing three inline spellings of the same literal.

  **BREAKING for checkpoint resume**: `compute_node_fingerprints` now fingerprints the dispatch field, so **a pre-fix checkpoint for any construct containing a `route="decide"` member invalidates on resume** (forced re-execution, or `CheckpointSchemaError` with no rewind point). Defensible at 0.x on the same terms as `neograph-ftnxl.5`'s `ordinal` addition, and stated here rather than discovered. That invalidation is also the POINT: before it, changing a Portal's `output=` contract changed the schema fingerprint (opening the resume gate) while leaving every per-node fingerprint byte-identical, so `_compute_invalidated_nodes` attributed the change to nobody, the empty set read as the documented genuine no-op, and the run **resumed from the tip with a stale dispatched result**. `state.py`'s comment claiming that field was "fingerprinted" so the change "correctly invalidates checkpoints" was false as written, and is gone.

  `tests/test_guards_write_set_monopoly.py` pins the monopoly, written failing-first against the pre-fix tree where it found exactly seven offenders. Its allowlist is shrink-only, every row carries a structural reason, and a stale row fails the build — it already caught `naming.py`, which defines the helper but never calls it. Measured across the tree: 20 field-name derivation sites before, 13 after, zero added outside the authority. Teaching lint the new field surfaced one false positive, which was fixed at `_framework_field_reads` — the documented single site for framework readers — rather than allowlisted: a dispatch node's success path exits through a synthetic node, so a later-declared `error_handler` takes the positional terminal slot and the dispatched result read as dead while being exactly what the run returns.

  **Found in passing, filed rather than folded in**: `verify_compiled` reports a false `state_field_missing` for every dict-form-output node in every graph (`neograph-ro330`); `Node.run_isolated` raises on a CORRECT dict-form node with a hint that blames the user for the one thing they did right (`neograph-ir50r`); the `@node` surface cannot consume a dict-form or dispatch producer at all, a three-surface parity gap (`neograph-lvdus`); `{node}_dispatch_error` is a real user-visible state field that NO derivation registers, including the validator (`neograph-qfq6v`); and a top-level first node declaring a single-type `inputs=` can never be fed and silently receives `None` on a green run — 67 measured sites, older and far more common than the bug this entry is about (`neograph-rfp5s`).

- **An empty `parametrize` is now a collection error, not a skip** (`neograph-e8wiv`). `tests/test_agent_spec_export.py` carried two tests parametrized over `FALLTHROUGH_COMBOS`, a set that is empty BECAUSE every fusion combo gained a real Agent Spec lowering -- the emptiness was the success condition. pytest renders an empty parameter set as `SKIPPED ... got empty parameter set for (combo)`, and `scripts/check_skips.py` -- correctly, since its allowlist was deleted -- cannot tell "empty because we succeeded" from "did not run because something broke". So the same defect the release-gate work was built to remove had reappeared in the one shape that gate could not read: two skips that were both harmless and indistinguishable from harm.

  Fixed at the source rather than per site. `empty_parameter_set_mark = "fail_at_collect"` in `pyproject.toml` makes pytest itself refuse an empty parameter set at COLLECTION, in every invocation including `make quality` -- so the condition can no longer reach the release gate at all, let alone pass it. An AST scan counts 55 `parametrize` marks in `tests/` whose argvalues is a derived expression rather than an inline literal; none needs a per-site guard now, and the emptiness of the one that mattered is stated as an assertion (`_assert_no_fallthrough`) that is also EXERCISED -- a new test moves `EACH_ORACLE` out of `SUPPORTED_COMBOS` in a simulated set and asserts the ratchet fires, which an absence could never do. The two tests that carried the empty marks are deleted, not converted: their bodies asserted the verbatim text of `_raise_no_agent_spec_lowering`, deleted in the same phase that emptied the set, so no input could reach them.

  `tests/test_guards_empty_parametrize.py` pins the setting by driving synthetic modules through a pytest subprocess pointed at the repo's own `pyproject.toml`. Its slip meta-test is the interesting one: pytest ids an empty parameter set as `NOTSET`, so the obvious hand-rolled guard -- match collected ids for `[NOTSET]` -- would also condemn `parametrize("name", ["NOTSET"])`, a real non-empty parametrize that collects with a byte-identical id. Keying on pytest's own emptiness branch makes that false positive structurally impossible.

- **A sub-construct's output boundary no longer resolves by scanning the child's whole state** (`neograph-35mur`, [GH #17](https://github.com/KonstantinMirin/neograph/issues/17)). `Construct.output=X` was not a wire but a SEARCH: the boundary value was found by walking the child's final state dict in reverse and returning the first `isinstance` match. That dict holds more than what the child computed — a forwarded `context=` field, `neo_subgraph_input`, framework keys — and `compile_state_model` appends context fields AFTER node output fields, so reverse iteration reached a value the child was HANDED before the one it PRODUCED. Reported shape: a branch declared `context=['read']` (putting a `Case` in its state) and `output=Case` because `Case` was genuinely the right domain type; every branch came back carrying the opening case, five readings and zero claims, and the run completed green. Note the action at a distance — `context=` is an INPUT declaration, and adding one silently re-pointed an unrelated OUTPUT with no error at assembly, compile, or run. The cost to the reporter was a type invented purely to be type-distinct, whose docstring had to say "the boundary type must be one that appears exactly once in a branch's state; distinctness is the whole of its job" — against this codebase's standing rule about types that only move shapes.

  **The fix is ELIGIBILITY, not a new declaration**: only fields the construct's OWN declared items write (via `iter_with_arms`) may be the boundary, last declared item first. A forwarded context field is not an item, so it can never win. Measured before choosing: 270 output-boundary resolutions across the suite, 26 with more than one type match, and this rule changes the answer on **2** — both of them the bug. The other 24 are ordinary same-typed chains (`review`→`revise`, `write`→`improve`) and resolve exactly as before, which is why ORDERING was chosen over refusing on ambiguity: a refusal would have broken the canonical refine sub-construct and pushed authors toward exactly the invented-for-distinctness type this fixes. It also converges with `_agent_spec_boundary.resolve_end_node_sources`, which already answers "which item is the boundary" positionally, instead of adding a fifth rule.

  **`Construct.output_from="settle"`** is the new EXPLICIT OVERRIDE for when position is wrong, or when you want the boundary stated rather than inferred. A name matching no item — or matching a forwarded context field, which is what the old scan used to return — refuses at `Construct(...)` itself, on the line where the typo is written. A SEPARATE field rather than letting `output=` take a str, deliberately: `Portal.output` and `ConstructSpec.output` already accept a str meaning a TYPE NAME via `lookup_type`, and one spelling with two contradictory meanings on sibling boundary fields is the same one-heuristic-two-rules condition that produced this bug. It is resolved eagerly to a type at assembly, so `_declared_output` keeps returning a type and its ~10 type-consuming call sites are untouched. Threaded through all three surfaces (declarative, programmatic, `construct_from_functions`), the YAML `ConstructSpec`, lint (a declared port names ONE terminal producer instead of every type-compatible one), and the Agent Spec round trip — a port is a member NAME with no Property representation, so without a `neograph/boundary_spec` marker export→import would drop it and the reimported construct would revert to the positional rule, green.

  The type scan SURVIVES for one reason that is not an oversight: a multi-arm branch's boundary is legitimately satisfied by a different node per arm (`output_reachable_on_every_arm`), which one name cannot express -- so the boundary is still resolved by TYPE, over the eligible set. There is no whole-state caller: `eligible` is a required argument of `_scan_subgraph_output`, and the structural guard's allowlist of whole-state callers is empty and shrink-only. Portal mode-(b) dispatch resolves its flow's boundary over the same eligible set, because the dispatched flow is built through `from_agent_spec` and normalized before it is invoked (`neograph-5zl3c`).

  **Found in passing, filed separately**: the same disease has three more sites, and two are new. `_extract_single_type` (`_input_shape.py`) resolves a single-type `inputs=` by scanning state FORWARD (earliest wins) while `_agent_spec.py`'s export mirror scans REVERSED (latest wins) — measured on one construct, the runtime consumes `a` while the exported artifact wires `sink <- b`, so **the exported Flow computes a different answer than the pipeline it came from** (`neograph-t1nbp`, P1). The input-port twin `_scan_subgraph_input` is `neograph-5suot`'s last open unknown and can adopt the eligibility half of this rule verbatim.
...
- **Tool ledger: per-tool ordinal stamping + `ToolLedger`** (`neograph-ftnxl.5`). `ToolInteraction` gains `ordinal: int` (1-based, per `tool_name`, per node; `0` means "not a budgeted invocation" — a handoff ack or a pre-0.8 checkpointed record) and a `key` property (`f"{tool_name}#{ordinal}"`, `None` when unaddressable). The ordinal is derived from the ALREADY-checkpointed `ToolBudgetTracker._counts` — no second counter. `neograph.ToolLedger` is a new pure read-time selector view (`first`/`last`/`all`/`grouped`/`by_key`) over an existing `list[ToolInteraction]`, e.g. a node's `tool_log` output. **BREAKING for checkpoint resume**: adding `ordinal` changes `compute_schema_fingerprint`/`compute_node_fingerprints` for any node declaring `tool_log` as a dict-form output, so a pre-`ftnxl.5` checkpoint invalidates on resume (forced re-execution, or `CheckpointSchemaError` with no rewind point) — defensible at 0.x, stated here rather than silent. **`typed_result` stays RESUME-VOLATILE** (documented and tested, not newly introduced): langgraph's checkpoint serializer flattens the `Any`-typed field via `model_dump()`, so a `BaseModel` result reads back as a plain `dict`; durable fields are `tool_name`/`args`/`result`/`duration_ms`/`ordinal` only. Also corrects a prior AGENTS.md claim: tool-call collection during the ReAct loop is unconditional, not demand-gated — only EXPOSURE to a node's declared output is.

- **`to_agent_spec(construct, api_provider=...)` — opt-in Agent Spec `LlmConfig.api_provider`** (`neograph-qtfof.8`). Every exported `LlmConfig` (think/agent/act nodes, and the Oracle `merge_prompt` merge node) previously had no way to set `api_provider`, so `pyagentspec`'s own LangGraph adapter raised `NotImplementedError: LlmConfig with api_provider='None' is not yet supported` for every LLM-bearing node — un-loadable/un-runnable by a third-party Agent Spec runtime. FAIL-LOUD OPT-IN, not a guessed default: neograph deliberately keeps the real LLM provider out of the graph layer (`Node.model` is an opaque tier string, resolved to a concrete `BaseChatModel` via `llm_factory` at runtime), so the exporter cannot infer a provider and must not fabricate one. `to_agent_spec(construct)` (no `api_provider=`) keeps today's honest behavior unchanged — no `api_provider` emitted, `export_conformance()` still reports `NEOGRAPH_ROUND_TRIP_ONLY` for these cells. `to_agent_spec(construct, api_provider="openai")` makes the same cells convertible. Existing pipelines and the zero-arg `EXEC_EXEMPT`/`COMPARE_CELLS` matrix in `tests/test_agent_spec_execute.py` are unaffected — this does NOT widen `COMPARE_CELLS` on its own, unlike `neograph-qtfof.6`/`.7`'s real-DataFlowEdge fixes.

- **The exported `EndNode` is finally wired: `neograph-qtfof.9`**. Every `to_agent_spec()` export synthesized a boundary `EndNode` with no `DataFlowEdge` ever feeding it — the outermost `Construct`'s `EndNode` declared no outputs at all (`construct.output` is always `None` there by design), so a third-party runtime's `invoke()` unconditionally returned `{}` regardless of what the graph computed; a NESTED sub-`Construct` with a declared `output=` fared worse, raising `Expected node to have a value for property X` since nothing fed its (correctly declared) `EndNode` either. `_agent_spec_boundary.resolve_end_node_sources` (new module — `_agent_spec.py` sits at its file-size ceiling) now wires the terminal producer (`construct.nodes[-1]`) into the `EndNode` with a real `DataFlowEdge`, for every Construct export, outermost and nested alike. Scoped to BARE/ORACLE terminal producers (an Each/Loop/Portal-wrapped terminal exports to a SpecNode whose own Property shape needs separate, modifier-aware resolution — falls back to today's behavior, tracked as a follow-up). A multi-exit-branch Construct (arms terminating in incompatible types) now fails LOUD at export (`ConfigurationError`) rather than guessing a union — a deliberate scope boundary, not a permanent limitation (`neograph-qtfof.14`). Along the way, fixed a related conformance-classifier gap: `export_conformance()`'s `LlmConfig` walker only checked bare `LlmNode`s, never `AgentNode.agent.llm_config` (agent/act mode) — invisible until this fix let those cells execute far enough to be empirically compared. Four cells (`scripted-bare-*`, `scripted-oracle-merge_fn-*`) are now the first ever to be genuinely `PORTABLE`, both statically and empirically.

- **`Carried` output-field marker, and a real fix to the existing `ExcludeFromOutput`** (`neograph-ftnxl.4`). `ExcludeFromOutput` documents itself as "the LLM won't try to produce it", but under the DEFAULT `output_strategy="structured"` that was false: the strip happened only inside `describe_type()` (the rendered prompt text), while `with_structured_output(...)` received the DECLARED model unprojected — an excluded field with no default was demanded of the provider as `required`, forcing the model to invent a value for a field the prompt never mentioned. `project_output_model()` is the fix: ONE projection (a structural sibling model, never a subclass — pydantic cannot remove an inherited field in a subclass) applied once in `invoke_structured`/`ainvoke_structured`, before either the rendered-schema preamble or the constrained-decode call, so the shown schema and the validating schema can never disagree again. `Carried(path)` joins the same projection for fields the FRAMEWORK supplies after parsing (spliced back in from the node's own declared `inputs`/DI params — never another node's output by an undeclared path, which would add a validator-invisible dataflow edge) — `Annotated[str, Carried("claims.source")]`. Assembly-time `ConstructError` covers the scope fence (undeclared root), depth-0-only (a marker on a nested model field), agent/act nodes (no splice mechanism there yet), and an Each/Loop-modified root (wrong runtime value shape for a flat splice); a missed splice never silently falls back to a Pydantic default.

- **Agent Spec export conformance classifier: `export_conformance()`** (`neograph-ftnxl.1`). `to_agent_spec()` not raising has been treated as proof a construct is portable to a third-party Agent Spec runtime, which is provably false in three tracked ways: `Each`/`Loop`/`Operator` fan-out and branch decisions live only in `neograph/*` metadata a foreign runtime ignores (`neograph-qtfof.6`/`.7`), an exported `LlmConfig` with `api_provider=None` fails to load under `pyagentspec`'s own loader (`neograph-qtfof.8`), and the outermost `Construct`'s `EndNode` declares no outputs by design, so a foreign `invoke()` never surfaces a result (`neograph-qtfof.9`). `export_conformance(construct, strict=True)` returns a `PORTABLE` / `NEOGRAPH_ROUND_TRIP_ONLY` / `NOT_EXPORTABLE` verdict, calibrated against the empirical EXECUTE+COMPARE tier (`tests/test_agent_spec_execute.py`'s real third-party-loader runs) rather than self-certified from "no predicate fired" — a static `PORTABLE` claim the empirical tier contradicts is a hard failure with no exemption. `to_agent_spec()`'s own signature and default behavior are unchanged (PORTABLE is empirically the empty set today, so a fail-loud default there would break all 223 existing call sites); `export_conformance()` is purely additive.

- **`context` gets a raw/typed escape hatch: `raw_context` — and the rendered-channel ratchet is now empty** (`neograph-ebxdg`). The last rendered prompt channel without a raw sibling. `context` is rendered text like `input_data` and `di_inputs`, but its claimed escape hatch — a model's `render_for_prompt()` override — only customizes the rendered *text* and is per-MODEL rather than per-NODE, so a compiler that needs a context value for LOGIC (iterating a model's children, branching on a field) rather than a `{var}` fill could never recover the typed object. `all_kwargs["raw_context"] = to_raw_inputs(context)` closes it, reusing `to_raw_inputs` verbatim and the same opt-in introspection gate — `DefaultPromptCompiler` deliberately does not declare it (it is a logic channel with no template-var meaning), so zero behavior change for every existing pipeline. This empties `NO_RAW_SIBLING_ALLOWLIST` in `tests/test_guards_prompt_channels.py`, the ratchet 0.7.6's `raw_di_inputs` work created; it may only stay empty. The guarantee is *"the object `context[k]` was rendered from"*, not the raw state value: a `Loop`-modified upstream yields the latest element, matching the rendered channel beside it.
...
- **The `structured` output strategy applies the null-to-default coercion `json_mode` already had** (`neograph-5s8f6`, [GH #20](https://github.com/KonstantinMirin/neograph/issues/20)). A Pydantic default applies only when a key is **absent**; a present-and-null value overrides it and fails validation, and models emit present-and-null routinely. neograph already repaired that — on one path only. `_apply_null_defaults` was reachable solely from `_parse_json_response`, the json_mode path; under `structured`, the DEFAULT strategy, validation happens inside LangChain, the resulting `parsing_error` was classified `Failed`, and the node burned its re-prompt budget and raised. Measured on the reporter's shape, a defaulted field one level down given a nested null: json_mode returned `Claim(root_cause=RootCause(changes=()))`, structured raised `ValidationError`. This is the same disagreement class as [GH #14](https://github.com/KonstantinMirin/neograph/issues/14), where the two strategies disagreed about which declared output types are usable at all, and the house rule established there is that they must agree about what a declared output can be. The coercion cluster moved to `_null_defaults.py` (`_llm_retry.py` was at its declared line ceiling, and this is the layout `develop` already uses), and `recover_null_defaults` re-validates the payload the provider **already sent** — read from `tool_calls[*].args` for `method="function_calling"` or from strict-JSON message content otherwise — so there is no extra round-trip. Strict JSON, never `repair_json`: a constrained decode emits machine-produced JSON, and the repairer could manufacture a payload out of prose. The hook sits in `_classify_lc_result`, the pure classifier the sync and async adapters already share, so both twins get it with zero duplication. Recovery fires **only** when the coercion actually changed the payload *and* the changed payload validates, so an unrelated validation failure is never masked — pinned by three tests asserting the re-prompt budget is still spent.

- **The null-default coercion descends into every homogeneous sequence, not only `list`** (`neograph-sjwny`, [GH #20](https://github.com/KonstantinMirin/neograph/issues/20)). The descent dispatched on `BaseModel`, `list` and `dict`, and a comment declared tuples out of scope because "LLM structured output does not emit heterogeneous tuples". True of `tuple[A, B]`, irrelevant to `tuple[X, ...]`, which is a list with a different constructor — and it is the container the reporter actually used, because a **frozen** Pydantic domain model uses tuples rather than lists. That is the same reasoning that widened `Each` in 0.7.9. So two byte-identical payloads got opposite treatment: under `list[Change]` a nested `{"kind": null}` coerced to the default, under `tuple[Change, ...]` it reached Pydantic raw and crashed on `root_cause.changes.0.kind`. `_sequence_item_annotation` is now the single homogeneous-sequence classifier — `list[X]`, `set[X]`, `frozenset[X]`, `tuple[X, ...]` and a fixed `tuple[X, X]` in; `tuple[A, B]` still out, which is what the original exclusion was actually about. The dispatch reads the annotation rather than the runtime container, because JSON decodes every array to a list. Found by running the reporter's shape one level deeper than the ticket described.

### Added

- **`lint()` reports a model-authored field whose default cannot survive a null** (`neograph-bek2f`, [GH #20](https://github.com/KonstantinMirin/neograph/issues/20)). `lint()` checked that the pipes connect — DI bindings resolve, placeholders resolve, supplied values reach something. All connectivity. It never asked whether a declared type can hold what the model will actually send. And neograph's own schema invites the failure: `describe_type` appends ` or null` to any field that carries a default and is not already nullable, so `status: ClaimStatus = "proposed"` reaches the model as `status: "proposed" or "accepted" or null`. Cost of not having the check, measured by the reporter across two paid runs on the same project in one day: 32.7M tokens at accuracy 0.904, then 28.0M at 0.000 with 192 errors, with their graph lint passing cleanly immediately before both. **The check recurses**, and that is the whole point — both failures were on types reachable *from* a node's output rather than on the output itself, one of them a `tuple[Change, ...] = ()` a level down, and the reporter's own first attempt walked top-level fields only and reported clean on a tree that already contained the defect. So the nested case is the first test in the file, the report names the dotted path (`root.changes`) so a top-level-only walk cannot fake a pass, the walker's entry is an annotation rather than a model so `outputs=list[Claim]` is not a silent hole, and `visited` is built fresh per node so a second node sharing a nested model is still reported. Scope is the declared **primary** output of `think`/`agent`/`act` nodes; scripted nodes are exempt because their output is built in Python, as are the framework-collected secondary keys of a dict-form `outputs=` and fields the schema never shows the model (`exclude=True` / `ExcludeFromOutput`). `WARN`, because the runtime now coerces the null back to the default — which is also why it is worth reading, since the model can decline to author the field and the default ships looking like an answer. `_admits_none` is imported from `describe_type`, never re-derived, and `tests/test_guards_nullability_marker.py` now scans the lint module alongside the renderer so the report and the prompt cannot describe different graphs.
