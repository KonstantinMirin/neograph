"""Keymaker mesh assembly validation (design §5) — a validation-cluster peer.

``_check_keymaker_mesh`` enforces every design-§5 mesh rule at ONE construct
...
def _member_keymaker(node: Node) -> Keymaker:
    """The Keymaker on a mesh member — non-None by construction.

    Members are collected by ``node.modifier_set.keymaker is not None``; this
    narrows the ``Keymaker | None`` slot back to ``Keymaker`` for the rules that
    read ``peers`` / ``route`` / ``model_fields_set``.
    """
    km = node.modifier_set.keymaker
    assert km is not None  # collected as Keymaker-modified
    return km


def _check_keymaker_mesh(construct: ConstructLike) -> None:
    """Validate the Keymaker mesh at ONE construct level (design §5).

    A **mesh** is the set of Keymaker-modified sibling Nodes at this level. This
    ...
    for item in nodes:
        if getattr(item, "name", None) == "handoff":
            raise ConstructError.build(
                "a node named 'handoff' collides with the reserved Keymaker mesh input key",
    ...
    for member in members:
        ...
        if not isinstance(member, Node):
            raise ConstructError.build(
                f"Keymaker mesh member '{name}' is a Construct",
                expected="mesh members must be sibling Nodes (D-MESH-LEVEL)",
                found="a sub-construct carries a Keymaker modifier",
                construct=construct.name,
                location=_source_location(),
            )
        if member.mode in ("agent", "act"):
            raise ConstructError.build(
                f"Keymaker mesh member '{name}' is an {member.mode}-mode node",
        ...
        if normalize_outputs(member.outputs).is_dict_form:
            raise ConstructError.build(
                f"Keymaker mesh member '{name}' declares dict-form outputs",
    ...
    # Past the shape checks every member is a Keymaker-modified Node (else we
    ...
    if hi - lo + 1 != len(member_positions):
        ...
        raise ConstructError.build(
            "Keymaker mesh members must be contiguous in the construct",
    ...
    for member in node_members[1:]:
        if _declared_output(member) is not payload:
            raise ConstructError.build(
                f"Keymaker mesh member '{member.name}' has a payload type that differs from the entry's",
    ...
    # Peers: every peer names a Keymaker-modified sibling.
    for member in node_members:
        for peer in _member_keymaker(member).peers or []:
            if peer not in sibling_names:
                raise ConstructError.build(
                    f"Keymaker member '{member.name}' names peer '{peer}' which does not exist",
            ...
            if peer not in member_names:
                raise ConstructError.build(
                    f"Keymaker member '{member.name}' names peer '{peer}' which is not Keymaker-modified",
                    expected="every peer must be a Keymaker mesh member",
                    found=f"'{peer}' is a plain (non-Keymaker) sibling",
                    node=member.name,
                    construct=construct.name,
                    location=_source_location(),
                )
    ...
    if seen != set(adjacency):
        ...
        raise ConstructError.build(
            "two disjoint Keymaker meshes at one construct level",
    ...
    for member in node_members[1:]:
        for knob in ("max_hops", "on_exhaust"):
            if knob in _member_keymaker(member).model_fields_set:
                raise ConstructError.build(
                    f"Keymaker member '{member.name}' sets {knob} but it is entry-only",
    ...
    route = _member_keymaker(entry).route
    ann = _resolve_field_annotation(payload, route)
    if ann is _MISSING:
        raise ConstructError.build(
            f"Keymaker route field '{route}' is missing from the payload model {_fmt_type(payload)}",
    ...
    if ann is str:
        ...
        for value in get_args(ann):
            if value not in valid:
                raise ConstructError.build(
                    f"Keymaker route Literal names '{value}' which is not a mesh member or HANDOFF_END",
        ...
        raise ConstructError.build(
            f"Keymaker route field '{route}' must be annotated str or Literal[...]",
    ...
    for member in node_members:
        ...
        if isinstance(inputs, dict) and "handoff" in inputs and inputs["handoff"] is not payload:
            raise ConstructError.build(
                f"Keymaker member '{member.name}' types its 'handoff' input as "
