...
- fields ``describe_type`` itself skips are skipped here too: the model never
  sees them, so it cannot send null for them. ``output_markers`` is the single
  predicate the text renderer and the schema projector already share, so
  ``ExcludeFromOutput`` AND ``Carried`` are both honoured -- a ``Carried`` field
  is one the LLM must not be asked to produce, so reporting it would be noise.
...
def _check_null_rejecting_defaults(item: Node, issues: list[LintIssue]) -> None:
    ...
    *visited* is built FRESH here, per node. Hoisting it across nodes would make
    ...
    if item.mode not in _MODEL_AUTHORED_MODES:
        return
    declared, _primary_key = resolve_primary_output(item)
    if declared is None:
        return
