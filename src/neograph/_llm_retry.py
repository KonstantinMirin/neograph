...
# The null-default coercion moved to its own module, neograph-5s8f6, so the
# structured path can share the SAME implementation instead of growing a second
# one. Re-exported here because this module was its only home for six releases
# and every existing import resolves through it.
from neograph._null_defaults import (  # noqa: F401
    _STRINGLY_NULL,
...
def _parse_json_response(text: str, output_model: Any) -> Any:
    """Parse LLM response text into the declared output type.
