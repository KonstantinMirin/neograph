...
def _gen_node_stubs(nodes: list[dict]) -> str:
    ...
    if scripted:
        ...
        for n in scripted:
            ...
            L.append(
                f'        pytest.fail("TODO: build input of {n["input_names"][0] if n["input_names"] else "no input"}, assert {n["outputs_name"]}")'
            )
            L.append("")
    ...
    if agents:
        ...
        for n in agents:
            ...
            if n["tools"]:
                L.append(
                    f"        Tools: {', '.join(t['name'] + '(budget=' + str(t['budget']) + ')' for t in n['tools'])}"
                )
            L.append('        """')
            for t in n["tools"]:
                L.append(f'        # register_tool_factory("{t["name"]}", lambda config: ...)')
