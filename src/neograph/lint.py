    ...
    *,
    ...
    for node in nodes:
        ...
        if inputs.is_dict_form:
            ...
            # producer's name, so record the type. Comparing a type-based
            # consumer against a name-keyed producer makes every such consumer
            # invisible and its upstream look dead.
    ...
    for node in nodes:
        ...
        for key, declared in (outputs.all_keys or {}).items() or (
            ...
            if root in consumed_whole or base in consumed_whole:
                continue
            # Consumed by a sub-construct's typed port.
