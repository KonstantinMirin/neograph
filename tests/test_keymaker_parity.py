"""KEYMAKER T4 — three-surface parity + @node sugar (neograph-kk262).

TDD RED artifact for the @node Keymaker sugar (``peers=`` / ``route=`` /
``max_hops=`` / ``on_exhaust=`` kwargs that build a ``Keymaker`` modifier, the
exact mirror of ``loop_when=`` -> ``Loop``). The sugar DOES NOT EXIST YET, so
every builder that assembles a mesh via ``@node(peers=...)`` — and every parity
/ conflict test that calls it — FAILS RED now (``peers=`` is an unexpected
...
Canonical mesh (all three surfaces): entry ``triage`` with ``peers=["billing"]``
and ``max_hops=6`` (the entry-only knob), terminal ``billing`` with ``peers=[]``
...
def _billing_logic(handoff: RouteHop) -> RouteHop:
    """Terminal specialist (peers=[]): routes only to HANDOFF_END."""
    return RouteHop(goto=HANDOFF_END, hops=handoff.hops + 1)
...
def _km_mesh_declarative(visits: list[str] | None = None) -> Construct:
    """Declarative surface: Node(...) constructor + | Keymaker."""
...
def _km_mesh_programmatic(visits: list[str] | None = None) -> Construct:
    """Programmatic surface: Node.scripted(...) | Keymaker."""
    sink: list[str] = visits if visits is not None else []
...
def _km_mesh_decorator(visits: list[str] | None = None) -> Construct:
    """@node decorator surface: peers=/max_hops= sugar building the Keymaker.

    RED NOW: ``peers=`` is not yet a recognized ``@node`` kwarg, so decorating
    ...
    @node(outputs=RouteHop, peers=["billing"], max_hops=6)
    def triage(handoff: RouteHop) -> RouteHop:
        sink.append("triage")
        return _triage_logic(handoff)

    @node(outputs=RouteHop, peers=[])
...
def _keymaker_of(construct: Construct, name: str) -> Keymaker:
    km = _node_by_name(construct, name).get_modifier(Keymaker)
    assert isinstance(km, Keymaker), f"{name} carries no Keymaker"
    return km


class TestThreeSurfaceKeymakerParity:
    """The same Keymaker mesh built via declarative ``Node(...)``, programmatic
    ``Node.scripted(...) | Keymaker``, and ``@node(peers=...)`` sugar produces
    IDENTICAL IR (Keymaker fields + model_fields_set, inferred handoff_param /
    ...
    ForwardConstruct.forward() to trace into a Keymaker. The OTHER three
    ...
    def test_keymaker_ir_identical_across_surfaces(self):
        """The Keymaker modifier itself — peers, route, max_hops, on_exhaust AND
        model_fields_set — is IDENTICAL across surfaces (per member).

        model_fields_set equality is LOAD-BEARING, not cosmetic:
        _validation_keymaker.py:189 reads it for the entry-only knob check, so a
        ...
        meshes = {bid: build() for bid, build in zip(_BUILDER_IDS, _ALL_BUILDERS, strict=True)}
        for member in ("triage", "billing"):
            kms = {bid: _keymaker_of(m, member) for bid, m in meshes.items()}
            peers = {bid: km.peers for bid, km in kms.items()}
...
class TestKeymakerDecoratorConflicts:
    """Decoration-time legality of the @node Keymaker sugar. These mirror the
    map_over/loop_when conflict raises (decorators.py:378) and fail RED now
    because ``peers=`` does not yet exist on ``@node``.
    """

    def test_peers_with_map_over_raises(self):
        """peers= and map_over= on one @node is a decoration-time conflict."""
        with pytest.raises(ConstructError):

            @node(outputs=RouteHop, peers=["billing"], map_over="upstream")
            def bad(handoff: RouteHop) -> RouteHop: ...

    def test_peers_with_loop_when_raises(self):
        """peers= and loop_when= on one @node is a decoration-time conflict."""
        with pytest.raises(ConstructError):

            @node(outputs=RouteHop, peers=["billing"], loop_when=lambda d: d is not None)
            def bad(handoff: RouteHop) -> RouteHop: ...

    def test_max_hops_without_peers_raises(self):
        """max_hops= without peers= is a decoration-time error (a peer-mode knob
    ...
    def test_route_without_peers_raises(self):
        """route= without peers= is a decoration-time error."""
        with pytest.raises(ConstructError):

            @node(outputs=RouteHop, route="goto")
            def bad(handoff: RouteHop) -> RouteHop: ...

    def test_on_exhaust_exit_routes_to_keymaker(self):
        """on_exhaust='exit' with peers= builds a Keymaker carrying on_exhaust=='exit'."""

        @node(outputs=RouteHop, peers=["billing"], on_exhaust="exit")
        def triage(handoff: RouteHop) -> RouteHop: ...
        ...
        km = _keymaker_of(mesh, "triage")
        assert km.on_exhaust == "exit"

    def test_on_exhaust_last_routes_to_loop(self):
        """on_exhaust='last' with loop_when= builds a Loop (not a Keymaker) —
        ...
        assert n.get_modifier(Keymaker) is None
