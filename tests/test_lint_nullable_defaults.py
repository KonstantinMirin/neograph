...
class CarriedFields(BaseModel):
    """develop-only: a ``Carried`` field is spliced in by the framework AFTER the
    response is parsed, so the model never authors it and never sees it in the
    schema. Reporting it would be pure noise -- the same class of false positive
    as ``ExcludeFromOutput``."""

    summary: str
    source_url: Annotated[str, Carried("seed.text")] = ""
...
class TestSilentCases:
    ...
    def test_no_issue_for_a_carried_field_when_the_framework_splices_it_in(self):
        """develop-only column: ``output_markers`` is the ONE predicate the text
        renderer and the schema projector share, and it strips ``Carried``
        alongside ``ExcludeFromOutput``. The lint check consults the same
        predicate, so a field the model is never asked to produce is never
        reported. Importing only ``_is_output_excluded`` here (the 0.7.x spelling)
        would report every Carried field."""
        carry = Node(
            "carry",
            mode="think",
            inputs={"seed": RawText},
            outputs=CarriedFields,
            model="fast",
            prompt="test/analyze",
        )
        assert _hits(_pipeline(_producer("seed", RawText), carry)) == []
