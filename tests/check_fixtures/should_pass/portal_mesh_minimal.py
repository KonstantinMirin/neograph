# A legal minimal Portal mesh: it ASSEMBLES cleanly (all §5 validation passes —
...
# (Command(goto) mesh via _add_portal_mesh). The check-fixtures harness compiles
...
pipeline = Construct(
    "swarm",
    nodes=[
        Node.scripted("triage", fn="km_f", outputs=Handoff) | Portal(to=["billing"], max_hops=6),
