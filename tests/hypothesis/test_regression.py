...
class TestYAMLSurfaceSharedNodeImmutability:
    ...
    def test_shared_node_in_two_yaml_sub_constructs(self):
        ...
        register_scripted("yaml_shared_worker", lambda _i, _c: SubOutput(result="from-worker"))
        ...
        sub_a = _build_sub_construct(
            ConstructSpec(name="sub-a", input="SubInput", output="SubOutput", nodes=["worker"]),
            all_nodes,
        )

        # Original in all_nodes must be unchanged (immutable IR)
        assert all_nodes["worker"].inputs == original_inputs, (
            f"_build_sub_construct mutated shared node! Was {original_inputs!r}, now {all_nodes['worker'].inputs!r}"
        )
    ...
    def test_yaml_sub_construct_node_gets_correct_inputs(self):
        ...
        register_scripted("yaml_step1", lambda _i, _c: SubInput(payload="step1-out"))
        register_scripted("yaml_step2", lambda _i, _c: SubOutput(result="step2-out"))

        spec = {
            "name": "yaml-2node-sub",
            "nodes": [
                {"name": "source", "mode": "scripted", "scripted_fn": "yaml_step1", "outputs": "SubInput"},
                {"name": "step1", "mode": "scripted", "scripted_fn": "yaml_step1", "outputs": "SubInput"},
                {"name": "step2", "mode": "scripted", "scripted_fn": "yaml_step2", "outputs": "SubOutput"},
            ],
            "constructs": [
                {"name": "process", "input": "SubInput", "output": "SubOutput", "nodes": ["step1", "step2"]},
            ],
...
class TestLLMSurfaceConfigPropagation:
    ...
    def test_merged_llm_config_reaches_factory(self):
        ...
        assert child.llm_config == original_config, f"Original mutated: {child.llm_config} != {original_config}"
...
class TestLLMSurfaceOracleCallCount:
    ...
    def test_oracle_calls_llm_n_times(self):
        ...
        pipeline = Construct(
            "llm-oracle-count",
            nodes=[
                Node("o-src", mode="think", model="fake", prompt="gen", outputs=Alpha),
                Node("o-gen", mode="think", model="fake", prompt="produce", inputs=Alpha, outputs=Beta)
        ...
        assert call_count[0] == 4, f"Expected 4 LLM calls (1 src + 3 oracle), got {call_count[0]}"
...
class TestCrossSurfaceValueEquivalence:
    ...
    def test_scripted_vs_decorator_produce_same_values(self):
        ...
        scripted_pipe = Construct(
            "equiv-scripted",
            nodes=[
                Node.scripted("equiv-src", fn="equiv_src", outputs=Alpha),
                Node.scripted("equiv-xform", fn="equiv_xform", inputs=Alpha, outputs=Beta),
            ],
        )
...
class TestOracleGenTypeIRParity:
    ...
    def test_oracle_gen_type_set_on_programmatic_oracle_with_merge_fn(self):
        ...
        ir = next(n for n in construct.nodes if getattr(n, "name", None) == "aqau-gen")
    ...
    def test_oracle_gen_type_set_on_yaml_oracle_with_merge_fn(self):
        ...
        spec = {
            "name": "aqau-yaml",
            "nodes": [
                {"name": "aqau-yaml-seed", "mode": "scripted", "scripted_fn": "aqau_yaml_seed", "outputs": "Alpha"},
                {
                    "name": "aqau-yaml-gen",
                    "mode": "scripted",
        ...
        construct = load_spec(spec)
        ir = next(n for n in construct.nodes if getattr(n, "name", None) == "aqau-yaml-gen")
        assert ir.oracle_gen_type is Beta, (
            f"YAML Oracle node must have oracle_gen_type=Beta; got {ir.oracle_gen_type!r}. Surface drift vs @node path."
        )
...
class TestFanOutParamIRParity:
    ...
    def test_fan_out_param_set_on_yaml_each_node(self):
        ...
        spec = {
            "name": "fpip-yaml",
            "nodes": [
                {"name": "fpip-src", "mode": "scripted", "scripted_fn": "fpip_src", "outputs": "FanCollection"},
                {
                    "name": "fpip-proc",
                    "mode": "scripted",
    ...
    def test_fan_out_param_set_on_programmatic_each_node(self):
        ...
        proc_ir = next(n for n in construct.nodes if getattr(n, "name", None) == "fpip-prog-proc")
    ...
    def test_each_keys_match_scripted_vs_yaml(self):
        ...
        scripted_pipe = Construct(
            ...
            nodes=[
                ...
                Node.scripted("ek-proc", fn="ek_proc", inputs=FanItem, outputs=Beta)
        ...
        spec = {
            "name": "ek-yaml",
            "nodes": [
                {"name": "ek-src-y", "mode": "scripted", "scripted_fn": "ek_src_y", "outputs": "FanCollection"},
                {
                    "name": "ek-proc-y",
                    "mode": "scripted",
    ...
    def test_loop_iters_match_programmatic_vs_forward(self):
        ...
        prog_pipe = Construct(
            ...
            nodes=[
                ...
                Node.scripted("lip-body", fn="lip_body", inputs=Beta, outputs=Beta)
        ...
        class LoopFwd(ForwardConstruct):
            ...
            body = Node.scripted("lif-body", fn="lif_body", inputs=Beta, outputs=Beta)
        ...
        assert prog_count[0] == fwd_count[0], f"Programmatic: {prog_count[0]} iters, Forward: {fwd_count[0]} iters"
...
class TestLLMSurfaceEachOracleCallCount:
    ...
    def test_each_oracle_calls_llm_items_times_n(self):
        ...
        pipeline = Construct(
            ...
            nodes=[
                Node(
                    ...
                    model="fake",
                    prompt="gen",
                    outputs=FanCollection,
                    llm_config={"output_strategy": "structured"},
                ),
                Node("eo-proc", mode="think", model="fake", prompt="produce", inputs=FanItem, outputs=Gamma)
        ...
        register_scripted("eo_src_scripted", lambda _i, _c: FanCollection(items=items))
        ...
        assert call_count[0] == 6, f"Expected 6 LLM calls (3 items × 2 oracle), got {call_count[0]}"
