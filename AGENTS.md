...
0. Output-shape checks — `model_authored_null_rejecting_default` (WARN). A field of a `think`/`agent`/`act` node's declared output that carries a default AND whose type rejects null. `describe_type` renders that field as `... or null`, so neograph's own schema tells the model it may send a value the declared type cannot hold; the runtime coerces the null back to the default, so the model can silently decline to author the field and the default ships looking like an answer. **The check recurses into nested models with cycle protection**, because both reported production failures were BELOW the declared output, not on it — a top-level-only walk reports clean on a tree that already contains the defect. Scripted nodes are exempt (their output is built in Python), as are the framework-collected secondary keys of a dict-form `outputs=` and fields the schema never shows the model (`exclude=True` / `ExcludeFromOutput`). `_admits_none` in `describe_type.py` is the nullability authority and is IMPORTED, never re-derived — `tests/test_guards_nullability_marker.py` scans both the renderer and the lint module so the report and the prompt cannot describe different graphs.
