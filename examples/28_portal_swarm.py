"""Example 28: Portal peer routing -- a typed swarm over declared specialists.

`Portal` turns a set of nodes into a *mesh*: each member declares a `peers`
...
no network -- but each `@node(portal=...)` stands in for a real agent stage. In
production `triage` would be `mode='think'` and a specialist `mode='agent'` with
tools; the `portal=` wiring is byte-identical.

This example covers mode (a) peer routing. Mode (b), dynamic flow dispatch
(`Portal(route='decide', ...)`) -- a member dispatches a whole sub-flow chosen
...
# specialist declares `portal=[]` and resolves straight to HANDOFF_END.
...
def demo_basic_routing() -> None:
    ...
    @node(outputs=Handoff, portal=["billing", "technical", "account"], max_hops=6)
    def triage(
        handoff: Handoff,
    ...
    # `billing` declares portal=["triage"] so it can bounce back (used in demo 2);
    # here we keep portal=[] so demo 1 stays a straight route. Build + run:
...
def demo_cycle() -> None:
    ...
    @node(outputs=Handoff, portal=["billing", "account"], max_hops=6)
    def triage(
        handoff: Handoff,
        subject: Annotated[str, FromInput] = "",
        body: Annotated[str, FromInput] = "",
    ...
    @node(outputs=Handoff, portal=["triage"])  # billing can hand BACK to triage
    def billing(handoff: Handoff) -> Handoff:
        if "charge" in handoff.body.lower():
            return Handoff(goto=HANDOFF_END, subject=handoff.subject, body=handoff.body,
                           resolution="Refund issued.", hops=handoff.hops + 1)
...
def demo_max_hops_budget() -> None:
    ...
    def _ping_pong(peers_exhaust: str):
        @node(outputs=Ping, portal=["pong"], max_hops=3, on_exhaust=peers_exhaust)  # type: ignore[arg-type]
...
# =============================================================================
# The `@node(portal=...)` sugar is one of three surfaces; declarative
# `Node.scripted(...) | Portal(...)` produces the IDENTICAL mesh IR and routing.
# The keyless scripted functions get their bodies through `compile(scripted=...)`.


def demo_surface_parity() -> None:
    print("=" * 68)
    print("DEMO 4 -- three-surface parity: declarative Node | Portal")
    print("=" * 68)
    ...
    print("same portal= wiring, same Command(goto) routing -- just a different surface.\n")
...
def main() -> None:
    ...
    demo_surface_parity()
    print("=" * 68)
    print("All PORTAL peer-routing surfaces ran: basic routing, genuine cycle,")
