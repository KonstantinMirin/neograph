...
def _prompt_compiler(template, data, *, di_inputs=None, **kw):
    ...
    ``di_inputs`` values are rendered text (neograph-l2a7w — di_inputs share the
    same rendering rule as every other channel and have no ``raw_inputs=``-style
    escape hatch, unlike ``data``), so ``history``/``dossier`` below are already
    prompt-ready strings, not the live ``EmailHistory``/``ActivityHistory``
    models — embed them directly rather than reaching for ``.emails``/``.events``.
    """
