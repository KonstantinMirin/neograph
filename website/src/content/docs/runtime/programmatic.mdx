...
{/* test-skip: illustrative fragment: references `decompose` defined in the page narrative, not in a runnable block */}
```python
...
{/* test-skip: snippet not runnable as written (SyntaxError: positional argument follows keyword argument (programmatic_105.py, line 4)) */}
```python
...
{/* test-skip: reads an illustrative file (`pipeline.yaml`) not present in the repo */}
```python
import yaml
