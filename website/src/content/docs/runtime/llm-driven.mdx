...
A second `Keymaker` mode -- dynamic flow dispatch, where a member runs a whole sub-flow chosen at runtime from a spec -- config-validates at assembly but is not yet dispatchable at runtime. The [Keymaker page](/concepts/keymaker/) documents it as a forthcoming surface.
