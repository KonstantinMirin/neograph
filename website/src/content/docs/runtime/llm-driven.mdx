...
- Insert human-in-the-loop checks on a node's output for sensitive results (interrupts fire at node boundaries, so this reviews a completed node's output — it does not gate individual tool calls inside an `agent`/`act` node; see [Human-in-the-Loop](/walkthrough/human-in-the-loop/))
