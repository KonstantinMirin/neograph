...
The `on_exhaust=` keyword is shared with `Loop` and routed by trigger: with `peers=` it feeds a `Portal` (which accepts `"error"` / `"exit"`), and with `loop_when=` it feeds a `Loop` (which accepts `"error"` / `"last"`). The `Portal` field itself is `Literal["error", "exit"]` -- `"last"` is a `Loop`-only value.
