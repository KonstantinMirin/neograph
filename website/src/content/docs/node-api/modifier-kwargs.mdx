...
Both `map_over` and `map_key` are required together. Passing one without the other raises `ConstructError`. To collect per-item failures instead of raising on the first one, add `map_on_error="collect"` — it forwards to `Each(on_error='collect')`.
...
|---|---|
| `merge_fn="name"` | Call a scripted function passed via `compile(..., scripted={"name": fn})`. Receives `(variants, config)`. |
...
| `merge_pre_process=fn` | (Optional, `merge_prompt` only) Transform variants into custom input_data for the prompt. |
| `merge_post_process=fn` | (Optional, `merge_prompt` only) Transform the LLM result before state write. |
| `merge_fallback=fn` | (Optional, `merge_prompt` only) Catch LLM errors, return deterministic result. |
...
{/* test-skip: illustrative fragment: references `UpstreamCtx` defined in the page narrative, not in a runnable block */}
```python
...
When using `merge_prompt`, optional hooks let you customize the merge without dropping down to `@merge_fn`:

{/* test-skip: illustrative fragment: references `DecompositionResult` defined in the page narrative, not in a runnable block */}
```python
def tag_with_gen_ids(variants: list[DecompositionResult]) -> dict:
    """Transform raw variants into custom prompt input."""
    tagged = [{"gen_id": f"gen-{i}", **v.model_dump()} for i, v in enumerate(variants)]
    return {"tagged_claims": tagged}

def ensure_all_grouped(result: GroupingResult, variants: list[DecompositionResult]) -> GroupingResult:
    """Self-healing: ensure every input claim appears in the output."""
    # ... validation and gap-filling logic
    return result

def singleton_fallback(variants: list[DecompositionResult], error: Exception) -> GroupingResult:
    """On LLM failure, each claim gets its own group."""
    return GroupingResult(groups=[Group(members=[v]) for v in variants])

@node(outputs=GroupingResult, prompt='rw/decompose', model='reason',
...
|------|-----------|-------------|
| `merge_pre_process` | `fn(variants: list[T]) -> dict` | Before LLM call. Replaces the default `{"variants": ..., **upstream}` input_data entirely. |
| `merge_post_process` | `fn(result: T, variants: list[T]) -> T` | After successful LLM call. Skipped when fallback fires. |
| `merge_fallback` | `fn(variants: list[T], error: Exception) -> T` | When `invoke_structured` raises. Returns a deterministic result. |

All three are optional and independent. They are only valid with `merge_prompt` -- passing any hook with `merge_fn` raises `ConfigurationError`. The programmatic equivalent:

{/* test-skip: illustrative fragment: references `Oracle` defined in the page narrative, not in a runnable block */}
```python
node | Oracle(n=3, merge_prompt="rw/group-claims",
              merge_pre_process=tag_with_gen_ids,
              merge_post_process=ensure_all_grouped,
              merge_fallback=singleton_fallback)
```
...
{/* test-skip: illustrative fragment: references `Summary` defined in the page narrative, not in a runnable block */}
```python
...
{/* test-skip: illustrative fragment: references `Analysis` defined in the page narrative, not in a runnable block */}
```python
from neograph import node, construct_from_module, compile, run
...
{/* test-skip: illustrative fragment: references `ValidationResult` defined in the page narrative, not in a runnable block */}
```python
...
# The string condition is resolved by the conditions= dict on compile():
graph = compile(pipeline, conditions={"needs_review": needs_review})
```

### When to reach for a raw interrupt instead

The Operator earns its place when the interrupt is **conditional** — the `interrupt_when=` predicate decides at runtime whether to pause, based on the state (a low coverage score, a failed validation, a risky action). That predicate is the value neograph adds.

If you only need an **unconditional** pause — always stop at this node for human sign-off — an Operator adds nothing over LangGraph's own `interrupt_before`. A predicate that returns a payload every time is just a static gate wearing a callable. For that case, prefer the engine's static breakpoint via the compile facade (`interrupt_before=["node_name"]`); it declares the gate to the engine directly instead of routing an always-true condition through the Operator machinery. Keep the Operator for the conditional case it exists to serve.
...
{/* test-skip: illustrative fragment: references `ReviewResult` defined in the page narrative, not in a runnable block */}
```python
...
### Scope projections: all_in_scope and from_enclosing(n)

A Loop's state field is already a full per-iteration history -- `_append_loop_result` appends on every iteration, never overwrites. Because `Loop` is sequential (not parallel like `Each`), list position *is* iteration order: element `i` (0-indexed) is always iteration `i + 1`'s output. Two named read-time projections fall out of that for free -- no extra storage, no schema change:

- **`all_in_scope`** -- declare the downstream parameter as `list[T]` instead of `T`. The full history passes through unchanged, in order.
- **`from_enclosing(n)`** -- `history[-n]`: the value from `n` iterations back. A plain negative index into the same list `all_in_scope` sees; no separate mechanism.
...
def summarize(refine: list[Draft]) -> Summary:
    # all_in_scope: refine is the FULL per-iteration history, not just the latest.
    return Summary(iterations_seen=[d.iteration for d in refine])
```

Without `all_in_scope` (i.e. a downstream parameter declared as the bare element type `Draft`), a consumer sees only the *latest* iteration's output -- the same unwrap a self-looping node sees on re-entry. Declaring `list[T]` switches to the full-history view. `from_enclosing(n)` is just `history[-n]` on top of that same list -- there's no dedicated helper function; it's documented shorthand for the slice, not new API surface.

This is a projection over the *default* Loop history — it does not attach per-iteration metadata (timestamp, condition-eval result). It also covers only a single, non-nested Loop: nested loops and cross-sub-construct scoping are separate, not-yet-solved problems, not something `all_in_scope`/`from_enclosing` extend to.
...
## Peer routing with peers, route, max_hops, and on_exhaust

The `peers=` keyword turns a node into a member of a runtime mesh -- it builds a `Portal` the same way `loop_when=` builds a `Loop`. Each member decides at runtime which declared peer to hand off to; neograph lowers the decision to a `Command(goto)`. See [Portal Peer Routing](/concepts/portal/) for the full model.

{/* test-skip: illustrative fragment: `Handoff` is defined in the Portal page narrative, not in a runnable block here */}
```python
@node(outputs=Handoff, peers=["billing", "technical"], max_hops=6)
...
| Keyword | What it does |
|---|---|
| `peers=["a", "b"]` | The declared peers this member may hand off to (directed, per-member). `peers=[]` makes a terminal member that can only route to `HANDOFF_END`. Passing `peers=` is what builds the `Portal`. |
| `route="goto"` | The field on the node's output payload whose value names the next peer. Defaults to `"goto"`. |
| `max_hops=6` | Caps the total hand-offs (the mesh analogue of `max_iterations`). An **entry-only** knob -- declare it on the member that starts the mesh, not on every peer. Defaults to 10. |
| `on_exhaust="error"` | What happens when the budget is spent: `"error"` (default) raises `ExecutionError`; `"exit"` leaves the mesh cleanly with the last payload. |

The `on_exhaust=` keyword is shared with `Loop` and routed by trigger: with `peers=` it feeds a `Portal` (which accepts `"error"` / `"exit"`), and with `loop_when=` it feeds a `Loop` (which accepts `"error"` / `"last"`). The `Portal` field itself is `Literal["error", "exit"]` -- `"last"` is a `Loop`-only value. Passing `on_exhaust=` with NEITHER a `loop_when=`/`portal=` trigger present is a decoration-time `ConstructError` -- the same is true of `max_iterations=` without `loop_when=`, `map_on_error=` (at a non-default value) without `map_over=`, and any of the four `merge_*` hooks without an Oracle trigger.

### Restrictions

- `peers=` cannot be combined with `map_over=` or `loop_when=` on the same node -- a member is either a mesh node, a fan-out, or a self-loop.
- `max_hops=` and `route=` require `peers=` -- they are peer-mode knobs with no meaning on a non-mesh node.
- An `Operator` may not sit on a mesh member. Gate a mesh with an interrupt placed BEFORE its entry or AFTER its exit instead.
...
{/* test-skip: snippet not runnable as written (CompileError: [Construct 'pipeline'] compile(): LLM-mode nodes (summarize) require runtime configuration   expected: llm_fac) */}
```python
from pydantic import BaseModel
from neograph import node, construct_from_functions, compile, run
...
Inject state fields into the LLM prompt alongside typed input. Unlike regular inputs, context values are not type-checked, and pre-formatted text passes through untouched -- `context=` is the channel for content you have already crafted for an LLM.
...
- Context values bypass the **renderer dispatch chain**: a configured `renderer=` (XML/JSON/delimited) is not applied to them, so a hand-written `<catalog>` reaches the model as `<catalog>` and not as escaped text. That is what makes `context=` the right channel for pre-formatted content like graph catalogs, domain briefings, or cached summaries.
- They are still **prompt-ready text** by the time a compiler sees them, like every other channel: a `str` is passed through byte-identical, a model's own `render_for_prompt()` is honored, and a model without one renders as BAML rather than as a Python `repr`. A context field naming a looping node yields that node's latest value, not its whole history.
...
| Rendered | Yes (BAML / renderer chain) | Text, but never renderer-wrapped: `str` passes through byte-identical; a presenter is honored |
...
A self-loop node's main output field is itself the full iteration history: for a
Loop node, `result[node]` is an append-list of every iteration's output (the last
element is the final value), so no separate history flag is needed.

```python
result = run(graph, input={"node_id": "writer"})
history = result["refine"]        # [Draft(v1), Draft(v2), Draft(v3)]
final = result["refine"][-1]      # the converged value
```
...
{/* test-skip: snippet not runnable as written (ConstructError: Duplicate Each modifier   found: An Each is already applied to this item   hint: Use a sub-construct if you ne) */}
```python
