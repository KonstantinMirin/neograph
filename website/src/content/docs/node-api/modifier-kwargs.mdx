...
## Loop history with Loop(history=True)

Collect every iteration's output into a separate history field for debugging or audit trails. History is an option on the `Loop` **modifier** (`Loop(history=True)`), not a `@node` kwarg -- apply it programmatically to the decorated node.

```python
from pydantic import BaseModel
from neograph import node, construct_from_functions, Loop, compile, run
...
# history= is a Loop-modifier option, not a @node kwarg. Apply it programmatically
# to the decorated node, then assemble the pipeline as usual.
refine = refine | Loop(
