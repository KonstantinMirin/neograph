...
The `peers=` keyword turns a node into a member of a runtime mesh -- it builds a `Keymaker` the same way `loop_when=` builds a `Loop`. Each member decides at runtime which declared peer to hand off to; neograph lowers the decision to a `Command(goto)`. See [Keymaker Peer Routing](/concepts/keymaker/) for the full model.

{/* test-skip: illustrative fragment: `Handoff` is defined in the Keymaker page narrative, not in a runnable block here */}
```python
...
| Keyword | What it does |
|---|---|
| `peers=["a", "b"]` | The declared peers this member may hand off to (directed, per-member). `peers=[]` makes a terminal member that can only route to `HANDOFF_END`. Passing `peers=` is what builds the `Keymaker`. |
...
The `on_exhaust=` keyword is shared with `Loop` and routed by trigger: with `peers=` it feeds a `Keymaker` (which accepts `"error"` / `"exit"`), and with `loop_when=` it feeds a `Loop` (which accepts `"error"` / `"last"`). The `Keymaker` field itself is `Literal["error", "exit"]` -- `"last"` is a `Loop`-only value.
