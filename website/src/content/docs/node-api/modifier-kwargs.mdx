...
## Loop history with loop_history=True

Collect every iteration's output into a separate history field for debugging or audit trails. Pass `loop_history=True` alongside `loop_when=` and the node's `Loop` modifier is composed with `history=True`; the programmatic form `Loop(history=True)` is equivalent. `loop_history=` requires `loop_when=` -- without a loop there is nothing to record, and the decorator raises `ConstructError`.
...
pipeline = construct_from_functions("writer", [seed, refine])
graph = compile(pipeline, checkpointer=InMemorySaver())
config = {"configurable": {"thread_id": "history-demo"}}
result = run(graph, input={"node_id": "history-demo"}, config=config)

# The main output is the append-list of every iteration.
final = result["refine"][-1]
print(f"Final: score={final.score}")

# The history lives in a framework field (neo_ prefix). run() strips neo_* fields
# from its result, so read it from the checkpointed state instead.
history = graph.get_state(config).values["neo_loop_history_refine"]
...
- **`Loop(history=True)`** creates an additional state field `neo_loop_history_{node_name}` that collects each iteration's output into a list.
...
- Because the field is framework-internal (`neo_` prefix), `run()` strips it from the returned result. Read it from the checkpointed state via `graph.get_state(config).values["neo_loop_history_{node}"]`, which requires compiling with a `checkpointer=`.
...
Fully programmatic form (runtime-constructed `Node`, no decorator):
