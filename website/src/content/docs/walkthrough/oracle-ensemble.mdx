...
The merge runs on the `reason` tier by default. To use a different model for the judge step, set it on the programmatic modifier — `Oracle(merge_prompt=..., merge_model="fast")` — or via the `merge_model:` key in the spec format; it is not a `@node` kwarg.
