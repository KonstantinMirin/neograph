...
graph LR
    start((" ")) --> extract
    extract -->|RawText| split
    split -->|Claims| classify
    classify -->|ClassifiedClaims| end_(((" ")))
```

Three nodes, linear sequence. Each edge carries a typed Pydantic model. The parameter name `extract` in `split(extract: RawText)` IS the edge — the compiler wires it automatically.
...
No `nodes=[...]` list, no ordering, no edge wiring. `construct_from_module` walks the current module, finds every pipeline member — `@node`-decorated functions, plain `Node(...)` instances, and sub-`Construct`s with an `output=` boundary — and topologically sorts them into a `Construct`:
