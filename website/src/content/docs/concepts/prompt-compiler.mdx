...
The other two override hooks are `load_template(template)` (swap the template source) and `build_vars(input_data, *, output_model, output_schema, di_inputs, context)` (change the variable namespace). Override exactly one; keep the rest.

`build_vars` layers its namespace in order of increasing specificity — `di_inputs` (run-wide ambient), then `context` (the node's declared context fields), then the rendered upstream outputs (the node's own dataflow). On a name collision the narrower binding wins. Write an override as `build_vars(self, input_data, **kw)` and forward `**kw` to `super()`: a compiler that spells out the keyword list keeps working, but has to be edited whenever the seam grows a channel.
...
def prompt_compiler(
    ...
    *,
    ...
    # input_data is ALWAYS a mapping of name -> prompt-ready text (`PromptInput`),
    # on every channel: node inputs, a single-type input keyed by its type name,
    # and the Oracle merge payload. There is no shape to branch on.
    template_vars = dict(input_data)
...
### When you need the objects, not the text

Every value in `input_data` is text. If a template needs to loop over a model's
children — `{% for c in claims %}` in one node and a flat summary in another —
declare a `raw_inputs` parameter and you receive the underlying objects
alongside the rendered text, keyed identically:

```python
from neograph import PromptInput

def prompt_compiler(template_name, input_data: PromptInput, *, raw_inputs, **kw):
    text  = input_data["claims"]   # always rendered text
    model = raw_inputs["claims"]   # the object behind it, because you asked
```

Like `di_inputs` and `context`, it is introspection-gated: a compiler that does
not declare it never receives it. Reaching for a field on a rendered value
instead raises `PromptInputError` rather than silently yielding `""`.

`di_inputs` has the same escape hatch: declare `raw_di_inputs` alongside
`di_inputs` to get the typed value a `FromInput`/`FromConfig` param resolved to,
not its rendered text — useful when a compiler branches on the value
(`isinstance(deal_id, int)`) rather than substituting it into a template:

```python
def prompt_compiler(template_name, input_data, *, di_inputs, raw_di_inputs, **kw):
    text = di_inputs["deal_id"]       # "4822" — always rendered text
    value = raw_di_inputs["deal_id"]  # 4822 — the int the DI resolver produced
```

`context` completes the set: declare `raw_context` to get the typed object each
context value was rendered *from*, for a compiler that iterates or branches on it
rather than substituting it:

```python
def prompt_compiler(template_name, input_data, *, context, raw_context, **kw):
    text = context["build_catalog"]     # rendered text, as the template sees it
    model = raw_context["build_catalog"]  # the model that text was rendered from
    n = len(model.items)
```

A model's `render_for_prompt()` override is not a substitute: it customizes the
rendered *text*, and it is per-model rather than per-node, so it never hands the
compiler the typed object. Note "rendered from", not "the raw state value" — a
context field resolves through the same upstream read the rendered channel uses,
so a `Loop`-modified upstream yields the latest element, not the loop history.
...
| `context` (kw) | `dict | None` | @node's `context=[...]` resolutions | Present when configured; rendered text |
| `di_inputs` (kw) | `dict | None` | Resolved `FromInput`/`FromConfig` params | Present for LLM-mode nodes with DI params; rendered text |
| `raw_inputs` (kw) | `dict` | Same source as `data` | Opt-in; the objects `data` was rendered from |
| `raw_di_inputs` (kw) | `dict` | Same source as `di_inputs` | Opt-in; the typed values `di_inputs` was rendered from |
| `raw_context` (kw) | `dict` | Same source as `context` | Opt-in; the objects `context` was rendered from |
