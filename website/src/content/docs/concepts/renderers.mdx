...
{/* test-skip: illustrative fragment: references `discover` defined in the page narrative, not in a runnable block */}
```python
...
### `dict[str, BaseModel]` -- merged fan-out

When an [`Each`](/concepts/each-oracle-fusion/) upstream's fanned-out results merge back onto the bus, a downstream node consuming them as a whole receives a `dict[str, BaseModel]` (result-key -> model). The BAML default renders **each value** via `describe_value()`, joined by a blank line:
...
### `list[ToolInteraction]` -- research packet

An `agent`/`act` node that declares a `tool_log` output produces a `list[ToolInteraction]`. When a downstream LLM node consumes it, the BAML default folds each interaction into a **research packet** -- a `{tool_name}:` header plus a body, entries joined by a `\n\n---\n\n` separator:
...
The body is `.result` (the pre-rendered form the agent loop always populates for its own tool calls) when non-empty, falling back to `describe_value(.typed_result)` when it is not -- a `ToolInteraction` you build by hand (e.g. a scripted node making its own tool call) typically sets only `typed_result`, and the fallback renders that instead of a blank body. The `tool_name` is kept as **provenance** -- a downstream node reasoning over a tool_log must know which tool produced each result. `args`/`duration_ms` stay omitted; they are bookkeeping, never the payload.
