---
title: Observability
description: Structured logging on every node, Annotated[T, FromConfig] for trace providers, and opt-in Langfuse tracing via observe=True.
---
...
{/* test-skip: illustrative fragment: references `Report`/`Claims`/`LangfuseTracer` defined in the page narrative, not in a runnable block */}
```python
...
{/* test-skip: illustrative fragment: references `graph`/`langfuse_tracer`/`my_limiter` defined in the page narrative, not in a runnable block */}
```python
result = run(
    graph,
    input={"node_id": "BR-042"},
...
[Langfuse](https://langfuse.com/) provides deep LLM call tracing -- prompt/completion pairs, token counts, latency waterfalls, cost tracking. NeoGraph wires it for you through `observe=`.

### One-line setup: `observe=True`
...
### Escape hatch: attach the handler yourself
...
{/* test-skip: illustrative fragment: references `my_langsmith_handler` defined in the page narrative, not in a runnable block */}
```python
