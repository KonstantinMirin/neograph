...
|---|---|---|
| **Tools** (bind) | Capabilities the *model* decides to call | `tools=` on `agent`/`act` nodes, filled per node from `mcp_tool_factories()` |
| **Tools** (compose) | Primitives *your code* calls in sequence | `mcp_session()` — one connection, N calls, typed/structured results |
| **Resources** | Data the *framework* fetches deterministically | `FromResource` DI marker, the resource manifest, `resource_reader` |
| **Prompts** | Templates the *server* curates | `mcp_prompt_source()` — a loader for `DefaultPromptCompiler`; the normal placeholder + lint rules apply |
...
- **Per-run identity.** The `token_provider` reads the operator from `config["configurable"]` on every run, so two runs of the same compiled graph act as two different principals. Over stdio the token rides as a framework-injected tool argument (stdio has no headers); over streamable-http it rides as a bearer header minted per request by an `httpx.Auth`. Identity is re-resolved **per call on every surface and transport** — per-request via the `httpx.Auth` over streamable-http, and re-injected per tool call over stdio — so nothing is frozen at connect: a provider that re-fetches a short-lived credential keeps working mid-run, while a static-string provider just keeps sending the same value. The token never enters state, the checkpoint, or the schema fingerprint.
