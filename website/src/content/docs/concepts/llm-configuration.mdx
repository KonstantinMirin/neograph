...
The framework tries signatures from most specific to least specific:
1. `(template, data, node_name=, config=, output_model=, llm_config=)` -- full context
2. `(template, data, node_name=, config=)` -- without output model
3. `(template, data)` -- minimal

This means you can upgrade incrementally without breaking existing compilers.
...
Both the factory and compiler support multiple signatures. The framework tries from most to least specific:

| Callback | Signatures (tried in order) |
|---|---|
| `llm_factory` | `(tier, node_name=, llm_config=)` then `(tier)` |
| `prompt_compiler` | `(template, data, node_name=, config=, output_model=, llm_config=)` then `(template, data, node_name=, config=)` then `(template, data)` |

Start with a simple lambda. Add parameters as you need them. No Node definitions need to change.
