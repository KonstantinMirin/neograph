...
{/* test-skip: snippet not runnable as written (ConstructError: [Node 'decompose'] mode='think' requires model=   hint: pass model='<model_name>' or switch to mode='scripted') */}
```python
@node(
    outputs=Claims,
    prompt="rw/decompose",
