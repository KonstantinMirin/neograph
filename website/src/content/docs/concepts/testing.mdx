...
The key pattern: pass the fake factory to `compile()` via the `llm_factory` and `prompt_compiler` kwargs. The configuration is scoped to that compiled graph -- no global state to register or reset between tests.
