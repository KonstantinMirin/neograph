...
2. **Replay.** If the *fetch* fails (an expired link), the only protocol-reliable way to re-derive a lifetime-free `resource_link` is to re-invoke the tool call that produced it. neograph reads a second consumer callable, the replayer, re-runs `ref.producing_call`, and fetches the fresh URI the replay emits.
