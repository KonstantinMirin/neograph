...
| `context` | No | State field names injected into the prompt as text; pre-formatted strings pass through untouched. |
...
#### Portal (dynamic handoff)

Two modes, matching the programmatic `Portal(...)` modifier field-for-field so a YAML mesh member and a `| Portal(...)` mesh member produce identical instances:

```yaml
# peer routing — a node picks its successor at runtime from a declared peer set
portal:
  to: [billing, technical]  # peer names (the mesh entry sets max_hops/on_exhaust)
  trigger: output           # "output" (default, typed goto field) or "tool"
  max_hops: 6                # ENTRY-ONLY — set on the mesh entry member only
  on_exhaust: error          # ENTRY-ONLY — "error" (default) or "exit"
  name: null                 # optional mesh group name
```

```yaml
# dynamic flow dispatch — the node emits the spec of the next flow to run
portal:
  route: decide              # literal "decide" selects dispatch mode
  spec_field: spec           # output field holding the emitted Spec dict
  input_field: dispatch_input
  output: Summary             # dispatched-flow output type (registered type name)
  max_depth: 3                 # required in dispatch mode
```

Only ONE of `to` (peer mode) or `route: decide` (dispatch mode) may be set. `max_hops`/`on_exhaust` are entry-only: setting either on a non-entry mesh member fails loud at assembly.

**Not YAML-expressible:** `scripted`/`conditions` are Python-only **callable registries** (`dict[str, Callable]`), omitted from `PortalSpec`. This is a real but narrow capability narrowing, not the earlier (incorrect) framing of "LLM-mode nodes only": `compile()` seeds a dispatch-mode Portal's emitted flow from every `@node`/`@merge_fn`-decorated shim and every `register_scripted`/`register_condition` registration independently of `scripted=`/`conditions=`, and an explicit kwarg only ever adds to or overrides that seed. What a YAML Portal cannot supply is a **portal-local** callable registry — one that provides a callable not globally registered, or that overrides a global registration for just this portal's emitted flow.

For a **spec-emitted flow** specifically, the condition half of that registry is reachable only through `operator.when` — `loop.when` is `parse_condition`-ed at LOAD TIME into a callable, so it never consults the emitted flow's condition registry at all. A dispatch-mode Portal's emitted flow with an `Operator` gate can reference a registered condition name; one with a `Loop` cannot reference a registry entry the same way, since the expression is resolved before the registry would ever be checked.
...
Sub-constructs support the same modifier blocks as nodes: `oracle`, `each`, `loop`, `operator`, `portal`.
...
{/* test-skip: illustrative fragment: references `load_spec` defined in the page narrative, not in a runnable block */}
```python
