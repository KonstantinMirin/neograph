...
Use `construct_from_functions(name, functions, input=X, outputs=Y)` to build a sub-construct from `@node`-decorated functions. Parameters whose type matches `input=` are automatically wired to the sub-construct's input port:
