...
- **Async streaming** -- the compiled graph delegates `astream()` for token/step streaming. (Sync `.stream()` and `.astream_events()` are not on the facade yet -- reach for the raw graph via `.graph` if you need them.)
