...
{/* test-skip: illustrative fragment: references `search_web` defined in the page narrative, not in a runnable block */}
```python
...
{/* test-skip: illustrative fragment: references `Jokes` defined in the page narrative, not in a runnable block */}
```python
import sys
from neograph import node, construct_from_module, compile, run
...
- **Streaming** -- drive the graph with the top-level `neograph.stream()` / `astream()` verbs for token/step streaming (`stream_mode` accepts a str or list, exactly like LangGraph). The facade also delegates `astream_events()` for event-level observability and `update_state()` / `aupdate_state()` for editing state before a HITL resume
- **Progress events** -- call `emit_progress(event)` from any node body to push a typed Pydantic event onto the `custom` stream
- **Inspection** -- the facade delegates `invoke`/`ainvoke`, `get_state`/`aget_state`, `get_state_history`/`aget_state_history`, and `get_graph`
- **Visualization** -- call `.get_graph().draw_mermaid()` on the compiled graph
- **LangGraph Platform** -- deploy the underlying LangGraph graph, reachable via `.graph` on the compiled result, with LangGraph Cloud or self-hosted
