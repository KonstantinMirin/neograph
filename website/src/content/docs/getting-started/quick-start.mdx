...
You got a compiled graph wrapping a standard LangGraph graph. You can async-stream it (`astream`), checkpoint it, visualize it, or deploy the underlying graph (via `.graph`) with LangGraph Platform -- NeoGraph does not change the runtime, only the authoring.
...
The compiled graph also delegates async facade verbs -- `astream` for step/token streaming and `aget_state` / `aget_state_history` for inspecting checkpointed state. See the [API reference](/reference/api/) for the full delegated set.
