...
Every field you pass in `input=` also flows into `config["configurable"]`, so your prompt compiler can read them via `config["configurable"]["topic"]`. This is how pipeline-level metadata (topic, node_id, project_root, whatever you need) reaches every node without threading it through state manually.
...
configure_llm(
    ...
    prompt_compiler=lambda template, data, **kwargs: [
        {"role": "user", "content": (
            # Read the topic from run() input via config injection
...
# --- Pipeline ---
decompose = Node("decompose", mode="produce", output=Claims, model="fast", prompt="decompose")
classify  = Node("classify",  mode="produce", input=Claims, output=ClassifiedClaims, model="fast", prompt="classify")
summarize = Node("summarize", mode="produce", input=ClassifiedClaims, output=Summary, model="fast", prompt="summarize")
...
- Fan out with `Oracle(n=3, merge_fn="...")` or `Each(over="previous_node.field", key="id")` -- see [Core Concepts](/concepts/vocabulary/)
