...
| A second async codebase | The same compiled graph runs under `run()` or `arun()` — the framework carries the sync/async twins |
| MCP client plumbing per node | `neograph[mcp]` turns a server config into per-tool factories you slice per node, with per-run identity |
| Resource fetch-and-parse glue | `FromResource` hydrates typed models at node entry, with size caps and self-healing expiry |
...
{/* test-skip: illustrative fragment: references `tools` defined in the page narrative, not in a runnable block */}
```python
...
{/* test-skip: illustrative fragment: references `StateGraph` defined in the page narrative, not in a runnable block */}
```python
...
- You need LangGraph features NeoGraph hasn't wrapped yet (custom reducers beyond what `Each`/`Oracle` cover, or streaming modes off the facade — `astream` is delegated, but sync `stream()` and `astream_events()` are not)
...
In those cases, use LangGraph directly. NeoGraph wraps a standard LangGraph graph underneath — you can always drop down to it via `.graph` on the compiled result to reach any method the facade doesn't delegate. The `Node(mode='raw', ...)` escape hatch exists for the same reason at the node level: a function with the standard `(state, config) -> dict` signature runs inside an otherwise @node-decorated pipeline.
